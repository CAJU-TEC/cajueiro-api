...
<head>
    ...
    <style>
        ...
        /* A cor vem da mesma classe chip-* do mês, para a data casar com o cabeçalho do card. */
        .day-badge {
            display: inline-block;
            color: #ffffff;
            font-size: 14px;
    ...
    <table class="header">
        <tr>
            <td class="header-brand">
                <div class="header-title">ANIVERSÁRIOS</div>
                <div class="header-title-big">CAJU</div>
                <div class="header-signature">CAJU<span>tec.</span></div>
            </td>
            <td class="header-art">
                <div class="blob">
                    @if ($payload['brand'])
                    ...
                    <td>
                        <div class="month-card">
                            ...
                            <div class="month-body">
                                @if (count($payload['birthdays'][$number]))
                                    <table class="person">
                                        @foreach ($payload['birthdays'][$number] as $person)
                                            <tr>
                                                <td class="person-name">
                                                    <span class="person-star">&#10035;</span>{{ $person['name'] }}
                                                </td>
                                                <td class="person-day">
                                                    <span class="day-badge {{ $chips[($number - 1) % 4] }}">
                                                        {{ $person['day'] }}
                                                    </span>
                                                </td>
                                            </tr>
                                ...
                                @else
                                    <div class="month-empty">NÃO HÁ<br>ANIVERSARIANTES</div>
                                @endif
                            </div>
                        </div>
                    </td>
